	...
	<script>
		...
		socket.on('msg1.vote.msg.{{$wall->id}}:App\\Events\\NewMessageVoteEvent', function (data)
		{
			...
			if(userid == data.user.id){
				if($("#iconholder"+data.message.id).hasClass("red")){
					$("#iconholder"+data.message.id).removeClass("red").addClass("blue");
				}
				else if($("#iconholder"+data.message.id).hasClass("blue")){
					$("#iconholder"+data.message.id).removeClass("blue").addClass("red");
				}
			}
			else{
		...
		socket.on('msg1.choice.polls.{{$wall->id}}:App\\Events\\NewPollChoiceEvent', function (data)
		{
			...
			var e = '<div id="choiceholder'+data.poll_choice.id+'" class="ui indicating progress success progress_jquery" data-percent="'+data.poll_choice.count+'">';
			e += '<div class="bar">';
